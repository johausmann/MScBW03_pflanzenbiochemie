...
* Plastid &rarr; Auto-Fluoreszenz der Phycobiline
  + Rotalgen ähnliche
...
```{r, echo=FALSE, fig.align='center',fig.cap="Allgemeiner Aufbau Thylakoidmembran"}
knitr::include_graphics("media/lecture4_allg_aufbau_thylakoid_membran.png")
```
  
* Variabilität in den Lichtsammelkomplexen

* Cyanophyta und Rhodophyta
  + Phycobilisom als Antenne auf PSI &rarr; Grund für einzelne Thylakoidmembranen
...
```{r, echo=FALSE, fig.align='center',fig.cap="Transport in komplexe Plastide"}
knitr::include_graphics("media/lecture4_komplexer_plastid.png")
```
...
* Äußere Membran mit ER des Wirts verbunden
  ...
  + Transitpeptid bestimmt weitere Lokalisation
    ...
    + Transport über "originale" envelope mittels TIC/TOC &rarr; TOC noch nicht gefunden, aber vieles deutet darauf hin
...
* Diatomeen sind Modellorganismen für sekundäre Endosymbionten
  * molekularbiologische Tools zur Untersuchung fortgeschritten
  
  * für 35% der Primärproduktion verantwortlich
  * besitzen keinen Nucleomorph
  * zwei Unterarten: Centrales und Pennales
  * Thylakoid 3er Stapel
  * Signalpeptide größtenteils bekannt
  
  * Transitpeptide wenig consensus mit höheren Pflanzen &rarr; Schwierigkeiten für Vorhersageprogramme
  
  * zwei Ansätze zur Untersuchung
      + Bioinformatisch (welche Gene, Signalpeptidde)  
      &rarr; Ableitung Stoffwechselwege und Kompartiment  
      + biochemisch/molekularbiologisch  
      &rarr; Nachweis des Protiens welches für bestimmte Funktion kodiert/Kompartiment
  
  
## Stoffwechselwege

* Diatomeen betreiben CO~2~ Fixierung
  *  besitzen gene für Carboanhydrase &rarr; CO~2~ (Gas) zu HCO~3~- (wasserlösl.) und umgekehrt
  * Enzyme für C4 Stoffwechsel vorhanden &rarr; Vorfixierung des Kohlenstoff
  * für Pflanzen an trockenen Orten &rarr; Vermeidung von Wasserverlust
  * bei Diatomeen kommt es aber nie zu HCO~3~- Knappheit
  
* Regulation Calvin Zyklus
  * nicht mittels Thioredoxin reguliert wie bei höheren Pflanzen
  * Calvin- und oxidativer Pentosephosphatweg unterschiedlich lokalisiert &rarr; gegeneinander Regulierung möglich, da ox. Pentosephosphatweg sich im Cytosol befindet und deshalb Thioredoxin nicht das erste Enzym hemmen muss
  
* Speicherassimilate
  * Diatomeen verwenden Chrysolaminarin (Polysaccharid) statt Stärke wie höhere Pflanzen
  * Chrysolaminarin wasserlöslich &rarr; in Vakuole gespeichert anstatt im Chloroplasten
  * An Tonoplast (Membran der Vakuole) synthetisiert
      + Export Chloroplast &rarr; Import Chloroplast 

* Glykolyse
  * Teilweise in Plastiden bis  (bei höheren Pflanzen kommt dies nur in den Wurzeln vor)
  * Problem: stört den Calvin Zyklus da 2PGA ein Intermediat des Calvinzyklus ist   
  &rarr; Regulation benötigt
  
## Molecular Tools für Diatomeen

### Ziele:
* einbringen zusätzlicher Gene
* Knock-Down mittels Antisense RNA
* Knock-Out via TALEN oder CRISPER-Cas9

### Biolistisch (Gen-Gun)
* Starke Selektion, da nur wenige Zellen überleben
  + darunter viele Falsch-Positive (Selektionsmarker integriert und GOI zerstört)
* Vektoren können leicht brechen &rarr; Scherkräfte
* stabiler Einbau in das Genom
  
### Elektroporation
* Variable Ergebnisse &rarr; funktioniert nicht bei allen Spezies
* Anzahl Transformanten höher
* stabiler Einbau in das Genom
  
### Episomen (Plasmid mittels *E. coli* einbringen)

* Plasmid enthält zwei ori &rarr; *E. coli* und Diatomeen
  + Replikation bei Zellteilung
* Plasmid benötigt 2 Selektionsmarker &rarr; *E. coli* und Diatomeen

* *E. coli* benötigt zusätzliches Plasmid um Plasmabrücken bei Kojugation auszubilden?
  + ToDo: Nochmal nachhören

* Selektionsdruck &rarr; transiente Transformation bzw. zeitlich begrenztes wirken
  + mit Wegfall des Selektionsdrucks &rarr; Verlust des Episomen und der eingebrachten Gene
  
## Knock-Out via TALEN (transkription activator-like effetor nucleases)

* Protein, welches an gewünschter Stelle schneidet (Endonuclease)
* Induziert Doppelstrangbrüche, welche mittels NHEJ repariert werden (fehleranfällig)
* Eingefügte Indels (!= 3n Basen) führen zu Knock-Outs
* TALE Sequenz muss so konzipiert werden, dass an gewünschter Stelle geschnitten wird &rarr; neue TALE Sequenz für jede Schnittstelle

## CRISPR-Cas9

* gRNA definiert Ziel der Schnittstelle
* induziert Doppelstrangbrüche
* Reparatur mittels NHEJ
* Bei insertions/deletions != 3n Basen &rarr; Knock-Out

* Off-Target Effekte &rarr; Cas9 schneidet an anderen Stellen
  + Dauerhafter Einbau unter Umständen nicht wünschenswert
  + Episom + CRISPR-Cas9 &rarr; zeitlich begrenztes wirken (keine dauerhafte Anwesenheit/Expression)

## Schwierigkeiten bei Diatomeen

* Homologe Rekombination möglich aber nicht einfach &rarr; Ersatz von genen schwer (wt Kopie immer dabei)

* Lange Regenerationszeiten

* Diplonten aber keine Kreuzung möglich (Bedingungen unklar und Zuordung zu "Geschlecht" schwierig) bzw. sehr schwer

* Bei Knock-Outs müssen beide Allele erfolgreich editiert werden, da Diatomeen diploid sind
  * Reparatur ist zu gewissem Grad zufällig: die Allele können unterschiedlich repariert werden

## Erhöhung Lipidproduktion

* Möglichkeiten der Produktionserhöhung:
  * Abbau verhindern
  * Aufbau fördern
  * konkurierende Reaktionen stoppen

* Diatomeen besitzen von Natur aus viel ungesättigte Fettsäuren &rarr; wichtig für Ernährung
* Prüfung, ob Tgl1 Lipase ist
  * in *E. coli* transferieren, aufreinigen und Enzymtest durchführen &rarr; Test ob Substrat umgesetzt wird
...
1. Sie entdecken einen unbekannten, einzelligen, photosynthetischen Organismus. Ihr Vergleich genomischer Sequenzen gibt Ihnen keine Verwandtschaft zu irgendeiner existierenden Algengruppe. Wie können Sie dennoch nachweisen, dass dieser Organismus  Chloroplasten besitzt, die aus einer sekundären Endosymbiose hervorgegangen sind? Bitte schreiben Sie in Stichworten Eigenschaften, die für sekundäre  Plastiden sprechen und eine  dafür geeignete Nachweismethode.

    + 3-4 Hüllmembranen mit CER und Verbindung zur Kern-ER &rarr; EM
    + Nukleomoprh &rarr; DAPI Färbung
    + Sequenzierung &rarr; Nachweis des zweigeteilten Chloroplasten Signalpeptid
        + S (bekannt) + T (geringer Consensus)
    + Thylakoidmorphologie &rarr; sekundäre Plastiden haben typische 3er Stapel
    + GFP mit Signalpeptid ausstatten &rarr; Co-Lokalisation mit Autofluoreszenz der Chloroplasten
    

2.  Welches Protein des PSII Komplexes muss ständig ersetzt werden, wo befindet es sich und welche Funktion bzw. Cofaktoren hat es? 
    + D1 des reaktiven Zentrums muss ständig ersetzt werden (30 min)
    + Teil des membranständigen Teils
    + Co-faktoren der Elektronentransportkette
        + P680 (Chlorophyll a), Lutein, $\beta$-Carotinoid

3. Das Photosystem II höherer Pflanzen liegt in typischen Superkomplexen in den  Granamembranen vor. Welche Proteine außer den Proteinen des PSII Cores sind darin in welchem Oligomerisierungszustand zu finden?

    + LHCII Komplexe &rarr; Timere
    + Minore LHCs (CP24, CP29, CP26) &rarr; Monomere
    + psbS &rarr; Dimer

4. Nennen Sie zwei der Faktoren, die die Synthese des Apoproteins des LHC 
beeinflussen.
    + PGE &rarr; GUN1 als Signalpeptid &rarr; aktiviert PTM Signalkaskade &rarr; Abi4 reguliert PHANG Expression
    + Chlorophyll-Synthese &rarr; Mg-Proto IX / Gun5 (Vorstufe) &rarr; verlässt Zellkern & reguliert mit Hsp90 &rarr; Hy5 reguliert PHANG Expression
    + Phytochrom &rarr; hellrotes Licht = aktive Form &rarr; Kernimport & Bindung von TF &rarr; Regulation von PHANG Expression
    
5. Welches Chlorophyll und welcher hauptsächliche Carotinoid dient bei Grünalgen, welches bei Diatomeen als akzessorisches Pigment? 
    + Alle besitzen Chlorophyll a
    + Grünalgen &rarr; Chlorophyll b & Lutein
    + Diatomeen &rarr; Chlorophyll c & Fucoxanthin

6. Nennen Sie drei Faktoren, die bei dem unter hohen Lichtintensitäten auftretenden non-photochemical quenching essentiell sind bzw. dazu beitragen. 

    + CP29-CP24-LHCII Superkomplexe
    + Aggregation von LHCII &rarr; Quenching über Lutein
    + Steigender Zeaxanthin und fallender Violaxanthin Gehalt
    + psbS
        + Modell 1 (Protonierung führt zu Konformationsänderung &rarr; Dissoziation von LHCII von PSII)
        + Modell 2 (LHCII + psbS (+ Zeaxanthin) Komplex quencht)

7. Warum ist die Nitratreduktion in den Chloroplasten extrem gut reguliert?

    + Pflanze nimmt Nitrat in höchst-oxidierter Form auf
        + Schrittweise Reduktion um Ammonium (NH~4~) zu erhalten
    + Nitrit ist starkes Oxidationsmittel
        + Ablauf nur wenn AS synthetisiert oder in Vakuole gespeichert
    + Ammonium auch toxisch &rarr; zerstört pH-Gradient
        + Muss sofort in AS Synthese verwendet werden
    + Zusätzlich Regulation mit CO~2~ und SO~4~ Assimilation

8. Nennen Sie zwei Stoffwechselwege, die oder deren Teile in Diatomeen eine ungewöhnliche Lokalisation innerhalb der Zelle aufweisen und nennen Sie die weit verbreitete Lokalisation sowie die Lokalisation in Diatomeen.

    + Photosynthese (Calvin Zyklus und oxidativer Pentosephosphatweg)
        + Weit verbreitet &rarr; Beides im Chloroplasten
        + Diatomeen &rarr; ox. Pentosephosphatweg im Cytosol
    + Speicherassimilat
        + Weit verbeitet &rarr; Stärke als Polysaccharid in Chloroplast
        + Diatomeen &rarr; Wasserlösliches Chrysolaminarin in Vakuole

9. Was ist der Vorteil plastidärer Transformation gegenüber nucleärer Transformation?

    + Plastiden werden __maternal vererbt__ &rarr; kein Pollenflug und Gefahr bei GVO
        + kein __vertikaler Gentransfer__
    + homologe Rekombination als Hauptreperaturmechanismus
        + einfaches Einbringen von plastidären Genen
    
10. Was ist der Nachteil plastidärer Transformation gegenüber nucleärer Transformation?
    + Gegenstrangkodierung und Operonstruktur können Probleme bereiten
    + Verwilderung von Pflanzen
    + Begrenzter Genpool für Ersetzung (~120)
    + Langwieriger Prozess &rarr; homoplastische Zellen
    
11. Beschreiben Sie in Stichworten den Weg zur Fertigstellung eines im Thylakoidlumen lokalisierten Proteins ohne Cofaktoren, welches nucleär codiert ist, beginnend mit dem Ort der Translation. 
    
    + Ort der Translation &rarr; Cytosol 
        + Protein am N-Temrinus mit Transitpeptid und Thylakoidtransferdomäne
    + Transport zur Chloroplastenmembran mittels Chaperonen
    + Transport über Membran (energieabhängig)
        + TOC &rarr; in Membraninnenraum transloziert
        + TIC &rarr; Transport in das Stroma
    + Abspalten des Transitpeptids
    + Transport zur Thylakoidmembran mittels Chaperonen
    + Transport über Thylakoidmembran
        + Ungefaltet = Sec-Komplex (energieabhängig)
        + Gefaltet = TAT-Komplex (pH-Gradient abhängig)

12. Worin unterscheiden sich die Chloroplasten von Rotalgen, Grünalgen und Braunalgen hinsichtlich ihrer Chloroplastenmorphologie?

    + Rotalgen
        + Einzelne Thylakoidmembranen (Bilayer)
        + Phycobilisom auf PSII aufgesetzt
        + 2 Hüllmembranen
    + Grünalgen
        + Stroma (einzel) und Granathylakoide (Stapel)
        + 2 Hüllmembranen
    + Braunalgen
        + Thylakoid 3er Stapel mit Gürtellamelle am Rand
        + 3-4 Hüllmembranen
    
13. Welche Protein-Pigment-Komplexe der Thylakoidmembran sind im Pflanzenreich hochkonserviert, welche variabel?

    + Grundsätzlich Aufbau der Thylakoidmembran mit PS, Cyt b~6~f und ATP-Synthase hoch konserviert
    + PSI und PSII hoch konserviert
        + PSI liegt bei Rotalgen und Cyanobakterien als Trimer vor, sonst Monomer
    + Lichtsammelkomplexe und gebunden Pigmente/Carotinoide sind hoch variabel
        + Rotalgen nutzen Phycobilisome (extrinsich)
        + Grünalgen nutzen LHC unterschiedlicher Art (Trimer, Minore)
        + Braunalgen nutzen FCP (Fucoxanthin)
        
14. Das Photosystem II höherer Pflanzen liegt in typischen Superkomplexen in den Granamembranen vor.

a) Welche Proteine außer den Proteinen des PSII Cores sind darin in welchem Oligomerisierungszustand zu finden?

    * Minore Lichtsammelantennen (CP24, CP26, CP29) &rarr; Monomere

    * LHCII {S,M,L} als Trimer

b) Welchen Oligomerisierungszustand weisen die Cores auf?

    * Reaktives Zentrum aus D1 & D2 als Heterodimer

    * CP43, CP47 als Monomere

15. Welche Substanz im Chloroplasten dient hauptsächlich als Signal zur Detektion von „Licht“ und „Dunkel“ und in welchem Zustand?

    + Thioredoxine regulieren viele Enzyme im Chloroplasten/Calvin-Zyklus
    + Reduzierte Form = Aktive Form (Cystinbrücke aufgebrochen)
    
16. Wie ist plastidäre DNA organisiert? Beschrieben Sie in Stichworten 3 typische Merkmale.
    + Organisation in Operons
    + Gegenstrangkodierung
    + Aufteilung in LSC und SSC. Inverted Repeats trennen SSC von LSC
